...
class UltraDatabase:
    def __init__(self):
        ...
        self.default_settings = {
            ...
            "anti_phone": True,
            "anti_email": True,
            "auto_ban_on_three_warnings": True,          # جدید: بن خودکار بعد از ۳ اخطار
            "two_factor_auth": False,                     # جدید: تأیید دو مرحله‌ای
            "daily_reward": True,                         # جدید: پاداش روزانه
            "daily_reward_amount": 10,                    # جدید: مقدار پاداش روزانه
            "auto_backup": True,                          # جدید: بکاپ خودکار
            "backup_interval": 86400,                     # جدید: فاصله زمانی بکاپ (ثانیه)
            "scan_media": True,                           # جدید: اسکن رسانه
            "malicious_domains": ["bit.ly", "tinyurl", "goo.gl", "ow.ly", "is.gd", "buff.ly", "adf.ly", "shorte.st", "cutt.ly", "rebrand.ly", "short.link"],
            "sensitivity_level": "normal",                # جدید: سطح حساسیت (low, normal, high)
            "duplicate_message_detection": True,          # جدید: تشخیص پیام تکراری
            "duplicate_time_window": 10,                  # جدید: پنجره زمانی برای تشخیص تکراری (ثانیه)
            "duplicate_threshold": 2,                     # جدید: تعداد تکرار مجاز
            "auto_report_to_admins": True                 # جدید: گزارش خودکار به ادمین‌ها
        }
        self.captcha = {}
        ...
        self.user_last_messages = defaultdict(lambda: deque(maxlen=10))  # برای تشخیص تکراری
        self.media_cache = {}
        self._load_stats()
    ...
    def add_message(self, user_id, text=None):
        """ثبت زمان و متن پیام برای تشخیص اسپم و تکراری"""
        now = time.time()
    ...
    def is_duplicate_message(self, user_id, text):
        ...
        if len(recent) < 2:
            return False
        # محاسبه شباهت با پیام‌های قبلی
        for prev in recent[:-1]:
            if self._text_similarity(text, prev) > 0.8:  # شباهت بالای ۸۰%
                return True
        return False
    ...
    def generate_2fa_code(self, user_id):
        ...
        user["twofa_expiry"] = int(time.time()) + 300  # ۵ دقیقه
        self.save_user(user)
        return code
    ...
    def is_malicious_domain(self, url):
        """بررسی دامنه‌های مخرب"""
        settings = self.get_group(0)  # فقط برای دسترسی به تنظیمات، اما باید group_id را پاس داد
        # برای سادگی از لیست داخلی استفاده می‌کنیم
    ...
    def scan_media(self, file_id):
        """اسکن رسانه (عکس، ویدئو) - فعلاً ساده"""
        # در اینجا می‌توان از APIهای تشخیص محتوا استفاده کرد
        # برای سادگی، فقط یک بررسی ساده انجام می‌دهیم
        return False  # به‌طور پیش‌فرض سالم است
...
def start_command(message):
    ...
    if message.chat.type in ['group', 'supergroup'] and not is_admin(user.id, chat_id) and not is_bot_admin(user.id):
        ...
        return
    text = f"""
✨ **ربات محافظ فوق‌پیشرفته Luffy Ultra Pro V2** ✨
━━━━━━━━━━━━━━━━━━━━━━
...
def daily_reward_command(message):
    ...
    if streak is None:
        ...
        return
    user = udb.get_user(user_id)
    xp_gain = 10 + (streak * 2)  # پاداش بیشتر با استریک بالاتر
...
def warn_command(message):
    ...
    try:
        ...
        if count >= warn_limit and not settings.get('auto_ban_on_three_warnings', True):
            ...
            udb._save_stats()
        elif count < 3:  # اگر به ۳ نرسیده، پیام نمایش داده شود
        ...
        # اگر به ۳ رسید و auto_ban فعال بود، پیام بن توسط add_warning ارسال شده است
...
def handle_new_member(chat_member_update: ChatMemberUpdated):
    ...
    if new.status == "member" and old.status in ["left", "kicked"]:
        ...
        if settings.get('anti_raid', True):
            ...
            # حذف زمان‌های قدیمی‌تر از ۱۰ ثانیه
            now = time.time()
...
def handle_message(message):
    ...
    # اگر پیام یک دستور ثبت شده باشد، نادیده گرفته می‌شود
    ...
    if settings.get('scan_media', True) and (message.photo or message.video or message.document):
        # در اینجا می‌توان از APIهای تشخیص محتوا استفاده کرد
        # برای سادگی، فقط یک هشدار می‌دهیم
        try:
...
if __name__ == "__main__":
    print("=" * 70)
    print("✨ ربات محافظ فوق‌پیشرفته Luffy Ultra Pro V2 ✨")
    print("=" * 70)
